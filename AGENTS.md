...
当前实现通过 `ContainerProps.OnTextInput`、`OnTextComposition`、`OnFocus`、`OnBlur`、`OnKeyDown` 驱动交互，文本片段使用 `TextProps.NoWrap` 保持单行。
...
- TextInput 已实现跨端基础单行编辑、IME 与字符级点击定位，但当前仍缺少剪贴板、文本选区、IME 候选窗精确跟随与横向滚动跟随光标。
