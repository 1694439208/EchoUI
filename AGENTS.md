...
- Hooks 状态系统：`State`、`Effect`、`Memo`、`Shared`、`MeasureText`、`IsInitialRender`。
...
- 内置元素与组合控件：`Container`、`Text`、`Input`、`TextInput`、`Button`、`CheckBox`、`ComboBox`、`RadioGroup`、`Switch`、`Tabs`、`Native`。
...
│   └── Elements/                 Button、Input、TextInput、Tabs 等组合/内置组件
...
通过当前 `IRenderer` 测量文本尺寸：

- 适合需要字符窗口、光标定位或轻量文本虚拟化的组件。
- 当前 `TextInput` 会用它计算可见文本片段，只渲染当前可见窗口。
- 该能力依赖当前 HookContext，因此也必须在组件 render 调用栈内使用。
...
- 浮动：`Float`。
- 动画描述：`Transitions`。
- 事件：点击、鼠标移动/进入/离开/按下/释放、键盘按下/抬起、文本输入、焦点获得/失去。
...
非原生单行输入框组件，由 `Container + Text` 组合实现，支持：

- 受控 `Value` 与 `OnValueChanged`。
...
当前实现通过 `ContainerProps.OnTextInput`、`OnTextComposition`、`OnPointerDown`、`OnPointerMove`、`OnPointerUp`、`OnFocus`、`OnBlur`、`OnKeyDown` 驱动交互，文本片段使用 `TextProps.NoWrap` 保持单行。
...
样式映射包括 flexbox、尺寸、margin/padding、背景、边框、溢出、transition、文本样式、input value 等。对于绑定键盘或焦点事件的 `Container`，WebRenderer 会设置 `tabindex` 以接收 DOM focus；`TextProps.NoWrap` 会映射为 `white-space: pre`。
...
- `Action`。
- `Action<string>`，用于 input 与 `ContainerProps.OnTextInput`。
- `Action<TextCompositionEvent>`，用于 `ContainerProps.OnTextComposition`。
- `Action<Point>`，用于鼠标移动。
- `Action<MouseEvent>`，用于 `ContainerProps.OnPointerDown` / `OnPointerMove` / `OnPointerUp`。
...
- 创建 Win32 窗口。
- 运行阻塞消息循环。
- 处理 `WM_PAINT`、`WM_SIZE`、鼠标、键盘、字符输入（`WM_CHAR`）、滚轮、`WM_COMMAND`、自定义更新消息。
...
- 支持按下、释放、点击。
- 支持向父链查找事件处理器。
- 支持把焦点赋给最近可输入祖先，并触发 `OnFocus` / `OnBlur`。
- 支持键盘事件与字符输入发给当前 focused element。
...
- 用户创建表单：TextInput、ComboBox、RadioGroup、Switch、Button。
- 最近用户列表。
- 系统状态卡片。
- 滚动主内容区。
...
- TextInput 已实现跨端基础单行编辑、IME、字符级点击定位、文本选区与基础右键菜单；当前仍缺少键盘快捷键级的复制/粘贴体验、IME 候选窗精确跟随与横向滚动跟随光标。
